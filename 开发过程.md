...
### 2026-02-10：移动中点击响应优化 (Click Responsiveness Fix)

#### 1. 问题现象
用户反馈在鼠标快速移动过程中点击预览窗口（即“甩动点击”），操作经常失效。这是因为 SwiftUI 的默认 `onTapGesture` 对鼠标位移非常敏感，一旦监测到微小移动就会判定为“拖拽”从而取消点击事件。

#### 2. 解决方案
- **手势替换**：在 `ThumbnailCardView` 中，使用 `DragGesture(minimumDistance: 0)` 替代 `onTapGesture`。
- **自定义判定**：在 `onEnded` 回调中手动计算鼠标释放位置。只要释放点在视图范围内（允许 20px 容差），即视为有效点击。
- **收益**：彻底修复了“移动中点击失效”的问题，大幅提升了操作的跟手度和容错率。
